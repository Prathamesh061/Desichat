...
import { postReq, get, deleteReq } from "../../utils/fetchData";
...
export const addMessage =
  ...
  async (dispatch) => {
    ...
    try {
      await postReq("message", msg, auth.token);
    } catch (err) {
      dispatch({
...
export const getConversations =
  ...
  async (dispatch) => {
    try {
      const res = await get(`conversations?limit=${page * 9}`, auth.token);
...
export const getMessages =
  ...
  async (dispatch) => {
    try {
      const res = await get(`message/${id}?limit=${page * 9}`, auth.token);
...
export const deleteMessages =
  ...
  async (dispatch) => {
    ...
    try {
      await deleteReq(`message/${msg._id}`, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const deleteConversation =
  ...
  async (dispatch) => {
    ...
    try {
      await deleteReq(`conversation/${id}`, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
