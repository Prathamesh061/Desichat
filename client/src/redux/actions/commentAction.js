...
import { postReq, patch, deleteReq } from "../../utils/fetchData";
import {
  createNotification,
  removeNotification,
...
export const createComment =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      const res = await postReq("comment", data, auth.token);
...
export const updateComment =
  ...
  async (dispatch) => {
    ...
    dispatch({ type: POST_TYPES.UPDATE_POST, payload: newPost });
    try {
      patch(`comment/${comment._id}`, { content }, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const likeComment =
  ...
  async (dispatch) => {
    ...
    try {
      await patch(`comment/${comment._id}/like`, null, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
...
export const unLikeComment =
  ...
  async (dispatch) => {
    ...
    try {
      await patch(`comment/${comment._id}/unlike`, null, auth.token);
    } catch (err) {
      dispatch({
        type: GLOBALTYPES.ALERT,
        payload: { error: err.response.data.msg },
  ...
  async (dispatch) => {
    ...
    try {
      deleteArr.forEach((item) => {
        deleteReq(`comment/${item._id}`, auth.token);
